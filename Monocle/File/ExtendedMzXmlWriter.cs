...
namespace Monocle.File {
  class ExtendedMzXmlWriter : MzXmlWriter
  {
    public void WriteScan(SpectrumEx scan)
    {
      ...
      writer.WriteAttributeString("polarity", scan.Polarity == true ? "+" : "-");
    ...
    /// <returns></returns>
    protected string EncodeLabelData(SpectrumEx scan)
    {
      if (scan.Count == 0 || (scan.DataPoints[0].Noise == 0 && scan.DataPoints[0].Baseline == 0))
      {
        return "AAAAAAAAAAA=";
      }
    ...
    protected string EncodePeaks(SpectrumEx scan)
    {
      if (scan.Count == 0)
      {
        return "AAAAAAAAAAA=";
