      ...
      await Task.Run(() =>
      {
        ...
        try
        {
          ...
          foreach (string newFile in files.FileList)
          {
            ...
            if (monocleOptions.OutputFileType == OutputFileType.mzml)
            {
              //TODO: Overload this in IScanWriter
              MzMLWriter writer = new MzMLWriter();
              writer.AddInstrumentConfiguration("IC1", null);
              writer.AddRun(outputFilePath, "IC1");
              Debug.WriteLine("Total scans = " + Scans.Count.ToString());
              foreach (Spectrum scan in Scans)
              {
                token.ThrowIfCancellationRequested();
                writer.AddSpectrum(scan);
              }
              writer.Write(outputFilePath + ".mzML");
            }
            else
            {
